...
#include "src/dto/stations/station_postgress.h"
...
void O::Controller::Network_Data_Handler::Rebuild_Network_String()
{
	...
	auto stations          = m_db->getStations()->template fetch<oatpp::Vector<oatpp::Object<O::DTO::Postgres_Station>>>();
	...
	auto lines             = m_db->getLines()->template fetch<oatpp::Vector<oatpp::Object<O::DTO::Line>>>();
	...
	if (lines) {
		for (auto& line : *lines) {
			...
			line->info_messages = To_List(
				m_db->getLineInfoMessages(lineId)->template fetch<oatpp::Vector<oatpp::Object<O::DTO::Info_Message>>>()
			);

			auto patternRows = m_db->getPatternsByLineId(lineId)->template fetch<oatpp::Vector<oatpp::Object<O::DTO::Pattern>>>();
			...
			if (patternRows) {
				for (auto& pattern : *patternRows) {
					pattern->info_messages = To_List(
						m_db->getPatternInfoMessages(pattern->id)->template fetch<oatpp::Vector<oatpp::Object<O::DTO::Info_Message>>>()
					);
					line->patterns->push_back(pattern);
				}
			}

			auto timetableRows = m_db->getTimetablesByLineId(lineId)->template fetch<oatpp::Vector<oatpp::Object<O::DTO::Service_Mission>>>();
			...
			if (timetableRows) {
				for (auto& timetable : *timetableRows) {
					timetable->info_messages = To_List(
						m_db->getTimetableInfoMessages(timetable->id)->template fetch<oatpp::Vector<oatpp::Object<O::DTO::Info_Message>>>()
					);
	...
	auto mapper = mappers->getMapper("application/json");
	m_full_network_json = mapper->writeToString(full_network);
