...
struct Line_Exemple
{
	...
	static void Test_Deserialize(const std::string& str)
	{
		...
		OATPP_ASSERT(pattern->arrival_times->size() == 3);
		auto it2 = pattern->arrival_times->begin();
		OATPP_ASSERT(it2->getPtr() == nullptr);
		OATPP_ASSERT(*(++it2) == 15);
		OATPP_ASSERT(*(++it2) == 45);

		OATPP_ASSERT(pattern->departure_times->size() == 3);
		auto it3 = pattern->departure_times->begin();
