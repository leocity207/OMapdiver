...
class Line_Info extends HTMLElement {
	 ...
	 */
	static Create(line_ID: string, network_data: Network) {
		const object = document.createElement('line-info') as Line_Info;
	...
	Render() {
		...
		const message_node = Utils.Get_Subnode(this.shadowRoot!, ".line-infomessages") as HTMLElement;
		...
		this.line_data!.patterns.forEach(pattern => {
			...
			schedules_node.appendChild(Line_Schedule.Create(pattern, this.network_data!, null));
		});
	}
}
