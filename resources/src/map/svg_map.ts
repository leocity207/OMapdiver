...
class SVG_Map {
	...
	*/
	Find_Map_Objs_By_Classname = (class_name: string, obj_type: string | undefined = undefined): any[] => {
		let result_list: any[] = [];
	...
	*/
	Animated_Pan_Zoom = async (zoom_box: Zoom_Box): Promise<void> => {
		...
		this.move_zoom_animation_obj.x = fabric.util.invertTransform(this.fabric_canvas.viewportTransform)[4];
		this.move_zoom_animation_obj.y = fabric.util.invertTransform(this.fabric_canvas.viewportTransform)[5];
		...
		await animate(this.move_zoom_animation_obj, {
			...
			onUpdate: () => {
				...
				this.fabric_canvas.absolutePan(new fabric.Point(
					...
					this.move_zoom_animation_obj.y
				));
	...
	async Initial_Zoom_Move() {
		...
		await animate(initial_zoom_move_obj, {
			...
			onUpdate: () => {
				...
				this.fabric_canvas!.absolutePan(new fabric.Point(initial_zoom_move_obj.x, initial_zoom_move_obj.y));
