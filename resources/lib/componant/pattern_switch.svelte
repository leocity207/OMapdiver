...
import { Pattern_Schemes } from "../../src/utils/networktype";
 ...
 */
class Pattern_Switch extends Observable(Toggleable(HTMLElement)) {
	...
	Set_State(state: any) {
		...
		this.setAttribute("current-state", this.current_index.toString());
		this.Render_State();
	 ...
	 * @param {boolean} open
	 */
	_Set_Dropdown_Open(open: boolean) {
