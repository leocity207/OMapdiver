...
import { Line, Station, Timetable } from "../types/network";
...
class TimeTable_Services_Missions extends HTMLElement {
	...
	Update(data: Line, stations: {[index: string]: Station}) {
		...
		this.m_display_state.calendar_pattern = "all";
		this.m_display_state.stop_pattern = "all"
	...
	Render() {
		...
		for (const timetable of (line.timetables ?? [])) {
			...
			let cal_tokens = Pattern_List_To_Tokens(timetable.calendar_pattern);
			let stop_tokens = Pattern_List_To_Tokens(timetable.stop_pattern);

			cal_tokens.push("all");
			stop_tokens.push("all");
