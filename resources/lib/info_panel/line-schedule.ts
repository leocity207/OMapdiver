...
import Fold_Plus_Minus from '../../src/components/fold-plus-minus';
import { Line, Network, Pattern } from '../types/network';
...
class Line_Schedule extends HTMLElement {
	 ...
	 */
	schedule_data: Pattern | null = null;
	 ...
	 */
	reference_station: string | null = null;

	/**
	 * the line data that we are displaying the data of
	 */
	reference_line: Line | null = null;
	 ...
	 */
	static Create(schedule_data: Pattern, network_data: Network, reference_station: string | null) {
		const object = document.createElement('line-schedule') as Line_Schedule;
	...
	connectedCallback() {
		...
		this.addEventListener('click', () => {
			...
			const fold = Utils.Get_Subnode(this.shadowRoot!,'plus-minus') as Fold_Plus_Minus;
			fold.Next_State();
	...
	Render() {
		...
		const header_info_icon = Utils.Get_Subnode(this.shadowRoot!,'.header-icon') as HTMLElement;
